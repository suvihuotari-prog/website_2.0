...
export const metadata = {
  title: "Asiakastarinat | Data Design",
  description: "Konkreettisia tuloksia suomalaisilta ja kansainvälisiltä asiakkailta. Tekoälystrategia, tiedonhallinta, asiakkuusäly ja ennakoiva toiminta.",
  alternates: {
    canonical: "/fi/customer-stories",
    languages: { en: "/customer-stories", fi: "/fi/customer-stories" },
  },
};

export default function CustomerStoriesPageFi() {
  const cases = getAllCases("fi");
  ...
  return (
    <>
      <HeroSection
        gradient="lemon"
        gradientRight
        badge="Asiakastarinat"
        title="Konkreettisia tuloksia johtavilta yrityksiltä"
        subtitle="Tekoälystrategiasta tiedonhallintaan, asiakkuusälystä ennakoivaan toimintaan. Katso, miten asiakkaamme muuttivat datan mitattavaksi liiketoiminta-arvoksi."
      />
      ...
      <CTASection
        heading="Haluatko vastaavia tuloksia organisaatiossanne?"
        subtitle="Varaa ilmainen 30 minuutin keskustelu. Käymme läpi, miten tekoäly voisi muuttaa liiketoimintaanne."
...
function StoryCard({ story, large = false }) {
  return (
    <Link
      href={`/fi/customer-stories/${story.slug}`}
      style={{
        display: "block",
        background: C.white,
    ...
    >
      ...
      <h3 style={{ fontSize: large ? 19 : 16, fontWeight: 500, color: C.black, lineHeight: 1.3, marginBottom: 12 }}>
        {story.title}
      </h3>
      ...
      <div style={{ fontSize: 14, fontWeight: 500, color: C.seawave, display: "flex", alignItems: "center", gap: 8 }}>
        Lue tarina
