...
export function ToolCard({ tool, ctaLabel = "Learn more" }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: 16, marginBottom: 14 }}>
        <div style={{
          width: 48, height: 48, borderRadius: 12, background: C.gray,
          display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0,
          transition: "transform 0.3s ease",
          transform: hovered ? "scale(1.05)" : "none",
        }}>{tool.icon}</div>
        <div>
          <p style={{ fontSize: 17, fontWeight: 500, marginBottom: 4, letterSpacing: "-0.01em", lineHeight: 1.4 }}>
            {tool.tagline}
          </p>
          <span style={{
      ...
      {tool.status && (
        <div style={{
          ...
          fontSize: 12, fontWeight: 500, color: C.seawave, marginBottom: 14,
        }}>
          <span style={{ width: 6, height: 6, borderRadius: "50%", background: C.seawave }} />
          {tool.status}
        </div>
      )}

      {/* Description */}
      <p style={{ fontSize: 14, lineHeight: 1.65, color: C.textMuted, marginBottom: 16, flex: 1 }}>
        {tool.description}
      </p>
